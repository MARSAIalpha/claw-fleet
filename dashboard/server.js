...
function getAllStatus() {
  ...
  if (fs.existsSync(heartbeatsDir)) {
    try {
      for (const file of fs.readdirSync(heartbeatsDir).filter(f => f.endsWith('.json') && !f.startsWith('_'))) {
        try {
...
const LATEST_HEARTBEAT_VERSION = 5;
...
<body>
  <div class="shell">
    ...
    <div class="overview-grid" id="overviewGrid">
      ...
      <div class="overview-card">
        <div class="ov-value ${(offlineCount+gwStoppedCount)>0?'ov-over':'ov-text'}" id="ov-abnormal">${offlineCount+gwStoppedCount}</div>
...
function executeRemoteCommand(agentId, command, callback) {
  ...
  const targetIp = agent?.local_ip
    || agent?.system?.tailscale_ip
    || (findMachineForAgent(agentId) || {}).tailscale_ip;

  const local = isLocalMachine(findMachineForAgent(agentId) || {});
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/api/dashboard-data' && req.method === 'GET') {
    ...
    const machinesData = agents.map(a => {
      ...
      const tu = oc.token_usage||{};
      return {
        agentId, isNever, isOk, online: a.online,
    ...
    return res.end(JSON.stringify({
      ...
      overview: { onlineCount, total, totalSessions, totalCronJobs, totalTokensToday: fmtT(totalTokensToday), totalCostToday: fmtC(totalCostToday), totalTokens30d: fmtT(totalTokens30d), totalCost30d: fmtC(totalCost30d), abnormal: offlineCount+gwStoppedCount },
