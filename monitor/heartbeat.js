...
function findOpenClawPath() {
  ...
  // 先试直接调用
  try {
    execSync('openclaw --version', { encoding: 'utf8', timeout: 5000, stdio: ['pipe','pipe','pipe'], windowsHide: true });
    return 'openclaw';
  } catch (e) {}
  ...
  if (isWin) {
    ...
    // 方法1: where 命令
    try {
      const result = execSync('where openclaw.cmd 2>nul', { encoding: 'utf8', timeout: 5000, windowsHide: true }).trim();
      if (result) {
    ...
    // 方法3: 从 npm prefix 查找
    try {
      const prefix = execSync('npm prefix -g', { encoding: 'utf8', timeout: 5000, windowsHide: true }).trim();
    ...
    // 方法4: npx 兜底
    try {
      execSync('npx openclaw --version', { encoding: 'utf8', timeout: 10000, stdio: ['pipe','pipe','pipe'], windowsHide: true });
      return 'npx openclaw';
    } catch (e) {}
  } else {
    // Mac / Linux
    try {
      const result = execSync('which openclaw 2>/dev/null', { encoding: 'utf8', timeout: 5000, windowsHide: true }).trim();
    ...
    const unixPaths = [
      ...
      '/usr/bin/openclaw',
      path.join(os.homedir(), '.nvm', 'versions', 'node', process.version, 'bin', 'openclaw')
    ];
    ...
    try {
      const prefix = execSync('npm prefix -g 2>/dev/null', { encoding: 'utf8', timeout: 5000, windowsHide: true }).trim();
      if (prefix) {
        let p = path.join(prefix, 'bin', 'openclaw');
        if (fs.existsSync(p)) return `"${p}"`;
      }
    } catch (e) {}
  }
...
function checkGatewayProcess() {
  ...
  try {
    const output = execSync(OC_CMD + ' gateway status', { encoding: 'utf8', timeout: 8000, stdio: ['pipe','pipe','pipe'], windowsHide: true });
...
// ── 检测 Telegram 是否可达（TCP 连通 api.telegram.org:443）──
// 返回 Promise<boolean>，3 秒超时
function checkTelegramReachable() {
  return new Promise((resolve) => {
    const net = require('net');
    const socket = net.createConnection({ host: 'api.telegram.org', port: 443, timeout: 3000 });
...
function getOpenClawVersion() {
  try {
    const { execSync } = require('child_process');
    return execSync(OC_CMD + ' --version', { encoding: 'utf8', timeout: 5000, windowsHide: true }).trim();
  } catch (e) {
    return 'unknown';
  }
}
...
function getOpenClawInfo() {
  ...
  try {
    ...
    if (myMachine && myMachine.bot_token) {
      ...
      try {
        const { execSync } = require('child_process');
        const current = execSync(OC_CMD + ' config get channels.telegram.botToken', { encoding: 'utf8', timeout: 5000, windowsHide: true }).trim();
...
async function heartbeat() {
  ...
  const gatewayAlive = checkGatewayProcess();
  // 只有 gateway 在跑时才检测 TG 连通性（节省 3 秒超时等待）
  const telegramReachable = gatewayAlive ? await checkTelegramReachable() : null;

  const status = {
    agent_id: AGENT_ID,
    heartbeat_version: HEARTBEAT_VERSION,
    timestamp: new Date().toISOString(),
    gateway_alive: gatewayAlive,
    telegram_reachable: telegramReachable,  // true=可达 false=被墙/断网 null=GW未运行
  ...
  const tgStatus = telegramReachable === null ? '(GW未运行)' : (telegramReachable ? 'TG✓' : 'TG✗');
  console.log(`[${status.timestamp}] Heartbeat: gw=${status.gateway_alive} ${tgStatus}, mem=${status.system.memory.usage_percent}%, models=${status.openclaw.models.length}, profiles=${Object.keys(status.openclaw.auth_profiles).length}`);
  ...
  if (req.url === '/api/command' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        // 修复 Mac/Linux 下 cron 环境没有 node/npm/openclaw 的 PATH 问题
        const env = Object.assign({}, process.env);
        if (!isWin) {
          env.PATH = `/usr/local/bin:/opt/homebrew/bin:/opt/homebrew/sbin:/opt/node/bin:${os.homedir()}/.nvm/versions/node/${process.version}/bin:${os.homedir()}/.npm-global/bin:` + (env.PATH || '/usr/bin:/bin:/usr/sbin:/sbin');
        }

        exec(cmd, { timeout: execTimeout, encoding: 'utf8', shell: isWin ? 'cmd.exe' : '/bin/sh', windowsHide: true, env }, (err, stdout, stderr) => {
...
function autoConfigBotToken() {
  try {
    ...
    // 魔法信箱补丁：每次启动时，强制解除群禁言（允许所有群回复）
    try {
      const { execSync } = require('child_process');
      execSync(`${oc} config set channels.telegram.groupPolicy open`, { encoding: 'utf8', timeout: 10000, windowsHide: true });
    } catch (e) {}

    // 检查当前配置的 token 是否已经一致
    const { execSync } = require('child_process');
    try {
      const current = execSync(`${oc} config get channels.telegram.botToken`, { encoding: 'utf8', timeout: 5000, windowsHide: true }).trim();
      if (current === token) {
    ...
    try {
      execSync(`${oc} config set channels.telegram.botToken "${token}"`, { encoding: 'utf8', timeout: 10000, windowsHide: true });
      // 魔法信箱补丁：强制设置允许在所有群聊说话
      execSync(`${oc} config set channels.telegram.groupPolicy open`, { encoding: 'utf8', timeout: 10000, windowsHide: true });
      console.log(`[BOT] Token 和 groupPolicy 已设置，正在重启 Gateway...`);
      execSync(`${oc} gateway restart`, { encoding: 'utf8', timeout: 15000, windowsHide: true });
